...
public class CommonUtils {
	...
	/* Date      A.E. Name       Description                                */
	/* --------  --------------  ------------------------------------------ */
	/* 11/19/22  R.Vattumilli    Initial Creation of validateTCData2()      */
	/*                                                                      */
	/************************************************************************/
	
	public static boolean validateTCData2(String idReq, String subsystemTC, String groupingTC, String bFunctionTC, String envTC) throws Exception {

		String validateTCInfo;
		int count;
		ArrayList<String> colValues  = new ArrayList<String>();
		ArrayList<String> colNames   = new ArrayList<String>();
		validateTCInfo = "select count(*) as count from requirement where trim(id_req) = '" + idReq + "'";
		colNames.add("COUNT");
		colValues = CommonUtils.executeQuery(validateTCInfo, colNames);
		count  = Integer.parseInt(colValues.get(0));
		if(count != 1 && !(idReq.equals(""))) {
			System.out.println(String.format(outputFormat, "Requirement Validation", reqTCValidationMessage));
			System.out.println(String.format(outputFormat, "Validation status", "FAIL"));
			return false;
		}
		validateTCInfo = "select count(*) as count from subsystem_area where trim(nam) = '" + subsystemTC + "'";
		colNames.add("COUNT");
		colValues = CommonUtils.executeQuery(validateTCInfo, colNames);
		count  = Integer.parseInt(colValues.get(0));
		if(count != 1) {
		...
		validateTCInfo = "select count(*) as count from test_case_grouping where trim(nam) = '" + groupingTC + "'";
		colNames.add("COUNT");
		colValues = CommonUtils.executeQuery(validateTCInfo, colNames);
		count  = Integer.parseInt(colValues.get(0));
		if(count != 1) {
		...
		validateTCInfo = "select count(*) as count from groop where trim(nam_group) = '" + bFunctionTC + "'";
		colNames.add("COUNT");
		colValues = CommonUtils.executeQuery(validateTCInfo, colNames);
		count  = Integer.parseInt(colValues.get(0));
		if(count != 1) {
		...
		validateTCInfo = "select count(*) as count from environment where trim(ind_environment) = '" + envTC + "'";
		colNames.add("COUNT");
		colValues = CommonUtils.executeQuery(validateTCInfo, colNames);
		count  = Integer.parseInt(colValues.get(0));
		if(count != 1) {
